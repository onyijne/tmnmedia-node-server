...
fs.writeFile(
  '/var/www/robot/web/reports/tader/trader.txt',
  'trader ok',
  function (err) {
    // if (err) throw err;
  }
...
fs.writeFile(
  '/var/www/robot/web/reports/trader/debug-tester.txt',
  'trader ok',
  function (err) {
    // if (err) throw err;
  }
...
fs.writeFile(
  '/var/www/robot/web/reports/trader/debuger.txt',
  'trader ok',
  function (err) {
    // if (err) throw err;
  }
...
export const robotWaker = async (req, res) => {
  ...
  try {
    ...
    logger(
      `${new Date()}: ${err.stack}`,
      '/var/www/robot/web/reports/trader/trade.txt'
    )
    response.message = err.message
    res.status(200).json(response)
  }
...
export const robotTrader = async (req, res) => {
  try {
    ...
    for (let i = users.length - 1; i >= 0; i--) {
      try {
        await DerivClient.initTrader(trade_options, users[i], settings, today)
      } catch (err) {
        await logger(err)
      }
    }
...
export const clearTrader = async (req, res) => {
  ...
  try {
    ...
    logger(
      `${new Date()}: ${err.stack}`,
      '/var/www/robot/web/reports/trader/trader.txt'
    )
    res.status(200).json({ message: err.message })
  }
}
...
export const robotWakeAll = async (req, res) => {
  try {
    ...
    for (let i = users.length - 1; i >= 0; i--) {
      try {
        await DerivClient.initTraderWaker(
          trade_options,
          users[i],
          settings,
          today
...
export const robotDigits = async (req, res) => {
  ...
  try {
    ...
    try {
      await DerivClient.initDigits(user, settings, trade_options)
...
export const listClients = (req, res) => {
  try {
    ...
    logger(
      `${new Date()}: ${err.stack}`,
      '/var/www/robot/web/reports/trader/robot.txt'
    )
    res.status(200).json({ message: err.message })
  }
}

export const mt5 = async (req, res) => {
  let response = {
    status: 'error',
    message: '',
  }
  try {
    const { action, wait_time, start_url } = req.body
    try {
      response = await DerivClient.mt5(action, wait_time, start_url)
    } catch (err) {
      await logger(err.stack)
      response.message = err.message
    }
    ...
    logger(
      `${new Date()}: ${err.stack}`,
      '/var/www/robot/web/reports/trader/mt5.txt'
    )
    response.message = err.message
    res.status(200).json(response)
  }
